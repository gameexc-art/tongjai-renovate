...
> ตั้ง env เพิ่ม `PARSE_MODEL=<ชื่อรุ่น>` ได้โดยไม่ต้องแก้โค้ด — **ต้องเป็นรุ่นที่รองรับ
> รูปภาพ (vision) บน Groq เท่านั้น** รุ่นข้อความล้วนจะอ่านสลิปไม่ได้ทุกใบ

---

## ขั้นที่ 4 — ที่เก็บประวัติ (✅ ตั้งค่าแล้ว — Vercel Blob)

โปรเจกต์นี้ **ต่อที่เก็บข้อมูลไว้เรียบร้อยแล้ว**: Vercel Blob store ชื่อ `tongjai-ledger`
(แบบ private) ถูกสร้างและ connect เข้าโปรเจ็กต์แล้ว โดย Vercel ใส่ env
`BLOB_READ_WRITE_TOKEN` ให้อัตโนมัติทุก environment — ไม่ต้องทำอะไรเพิ่ม

- แต่ละรายการถูกเก็บเป็นไฟล์ JSON แยกใบ (`ledger/rec/<รหัสกันซ้ำ>.json`) ไม่มีการเขียนทับ
  จึงกันบันทึกซ้ำได้แบบ atomic ทั้งจาก redelivery ของ LINE และการกดปุ่มยืนยันซ้ำ
- ระบบยังรองรับ **Vercel KV / Upstash Redis** (`KV_REST_API_URL` + `KV_REST_API_TOKEN`)
  เป็นทางเลือก — ถ้าตั้งไว้ทั้งคู่ KV จะถูกใช้ก่อน
- ถ้าไม่มีที่เก็บเลย: บอทยังทำงานครบทุกขั้น เพียงข้ามการบันทึกอย่างนุ่มนวล และ
  `/api/transactions` จะตอบรายการว่าง
...
| `PARSE_MODEL` | ชื่อรุ่น vision บน Groq (ต้องรองรับรูปภาพ) | ไม่ (มีค่า default ให้) |
...
| `BLOB_READ_WRITE_TOKEN` | ✅ ใส่ให้อัตโนมัติแล้วตอน connect Blob store (ขั้นที่ 4) | เก็บประวัติ (ตั้งแล้ว) |
| `KV_REST_API_URL` | ทางเลือกแทน Blob (Upstash Redis REST) | ไม่ |
| `KV_REST_API_TOKEN` | ทางเลือกแทน Blob | ไม่ |
...
6. กดเลือก → บอทยืนยัน **“✅ บันทึกเป็นใบรับรองแทนแล้วครับ”**
   (ถ้าเขียนลงที่เก็บไม่สำเร็จ บอทจะบอกว่า **“บันทึกไม่สำเร็จ ลองกดยืนยันอีกครั้งนะครับ”**
   ให้กดซ้ำได้เลย — ระบบกันบันทึกซ้ำต่อสลิปให้อยู่แล้ว)
7. ดูรายการที่บันทึกไว้ผ่าน `/api/transactions`
...
- เปิดมาจะเจอ **ช่องใส่รหัสผ่าน** — ใส่ค่าเดียวกับ `DASHBOARD_TOKEN` ที่ตั้งไว้ในขั้นที่ 5
  ...
  และมีปุ่ม **ออกจากระบบ** ให้ล้างได้)
...
- ใส่รหัสผิดจะขึ้น **“รหัสไม่ถูกต้อง”** ให้ลองใหม่ได้ (ถ้าเผลอพิมพ์ทั้งที่แป้นพิมพ์เป็น
  ภาษาไทย หน้าจะเตือนให้เช็คภาษาแป้นพิมพ์ทันที)
- ที่เก็บประวัติ (Vercel Blob) **ตั้งค่าแล้ว** — ส่งสลิปเข้าบอทให้ครบขั้นตอน รายการจะโผล่ที่นี่
  ตราบใดที่ยังไม่มีรายการ หน้าจะขึ้น
...
| การ์ด/ปุ่มขึ้น แต่กดแล้ว **ไม่บันทึก** | • ขึ้น **“บันทึกไม่สำเร็จ ลองกดยืนยันอีกครั้ง”** = เขียนลงที่เก็บพลาดชั่วคราว (Blob/KV ขัดข้อง) → กดยืนยันซ้ำได้เลย (ระบบกันบันทึกซ้ำต่อสลิปให้แล้ว) • ถ้าไม่มีที่เก็บเลย (ไม่มีทั้ง `BLOB_READ_WRITE_TOKEN` และ `KV_REST_API_*`) บอทจะทำงานปกติแต่ไม่เก็บประวัติ |
| `/api/transactions` คืน `401` | ไม่ได้แนบ/แนบ `DASHBOARD_TOKEN` ผิดในเฮดเดอร์ `Authorization: Bearer …` (หรือใช้เฮดเดอร์ `x-dashboard-token` ก็ได้) |
| `/api/transactions` คืน `503` | ยังไม่ได้ตั้ง env `DASHBOARD_TOKEN` (endpoint ปิดไว้จนกว่าจะตั้งรหัส) |
| `/api/transactions` คืน `502` | ที่เก็บข้อมูล (Blob/KV) ขัดข้องชั่วคราว — ข้อมูลไม่หาย ลองรีเฟรชอีกครั้ง (ตั้งใจตอบ error แทนที่จะแกล้งตอบลิสต์ว่าง) |
| `/api/transactions` คืน `transactions: []` | ยังไม่มีรายการที่บันทึกครบขั้นตอน (หรือไม่มีที่เก็บ configured) |
...
- บอทกันบันทึกซ้ำ **ทั้งจาก redelivery ของ LINE และการกดปุ่มยืนยันซ้ำ** — ทุกสลิปมี
  รหัสกันซ้ำจากตัวรูป (hash ของ messageId) ผูกถึงชั้นที่เก็บ (Blob: ไฟล์ต่อรายการแบบ
  ห้ามเขียนทับ / KV: `SET NX`)
...
## พ่วง Claude Code usage เข้ากับ NONEAICE

นอกจากอ่านสลิปแล้ว NONEAICE ยังเช็ค **การใช้งาน Claude Code** ให้ได้ด้วย —
พิมพ์ `usage` / `เช็ค` / `status` / `/status` / `ยอดใช้` ในแชท (ทั้งใน **LINE** และ
**หน้าเว็บ Screen B**) แล้วบอทจะตอบเป็น **การ์ดสรุป usage** (session 5 ชม. / weekly /
weekly Sonnet พร้อมแถบเปอร์เซ็นต์และเวลารีเซ็ต)

**เงื่อนไขก่อนใช้:** ระบบ usage bot ของโปรเจกต์ `apartment-dashboard` ต้อง **sync snapshot
อยู่แล้ว** (สคริปต์ `line-usage-bot/sync_to_vercel.py` รันบนเครื่องที่ใช้ Claude Code)
— แอปนั้นจะเสิร์ฟข้อมูลล่าสุดที่ `/api/usage/snapshot` ให้บอทนี้มาดึงต่อ
บอท **ไม่เรียก Claude/Anthropic เอง** เลย จึงไม่กินโควต้าเพิ่ม

**วิธีตั้งค่า (ครั้งเดียว):**

1. ไปที่ Vercel → โปรเจกต์ `tongjai-renovate` → **Settings → Environment Variables** เพิ่ม:

   | ชื่อ env | ค่า | บังคับ? |
   |---|---|---|
   | `USAGE_SNAPSHOT_URL` | `https://<แอป apartment-dashboard ของคุณ>.vercel.app/api/usage/snapshot` | ✅ ใช่ (สำหรับฟีเจอร์นี้) |
   | `USAGE_KEYWORDS` | รายการคำสั่งเอง คั่นด้วยจุลภาค เช่น `usage,เช็ค,quota` | ไม่ (มีชุด default ให้แล้ว) |

2. **Redeploy** โปรเจกต์ 1 ครั้งเพื่อให้ env มีผล

**ทดสอบ:** พิมพ์ `usage` ใส่บอท — ควรได้การ์ดสรุปทันที
ถ้า **ยังไม่ได้ตั้ง** `USAGE_SNAPSHOT_URL` บอทจะตอบข้อความไทยแนะนำวิธีตั้งค่าแทน
(ไม่พัง ไม่เงียบ) และถ้าปลายทางล่มชั่วคราวจะตอบ **“ดึงข้อมูล usage ไม่สำเร็จ
ลองใหม่อีกครั้งนะครับ 🙏”** — ลองซ้ำได้เลย

> ทิป: ฝั่งหน้าเว็บมี endpoint เดียวกันที่ `GET /api/usage` (สาธารณะ อ่านอย่างเดียว) —
> ถ้ายังไม่ตั้ง env จะคืน `503`, ปลายทางล่มคืน `502`
